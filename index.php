<?php include_once 'lib/function.php'; 
 ...
 if ($_POST) {
    ...
    $comparer =  $pdo->query('SELECT * FROM user WHERE login = "'.$login.'"');
    $result = $comparer->rowCount();
    
    if ($result == 0) {
        
        echo '<h3>Vous n\'est pas un stilisateur de INSTAMAILMEDIA si vos appartenez a la Societe Envoyez Nous un Mail</h">';
    } else {
        
        $password = htmlentities($_POST['password']);
        $comparerP = $pdo->query('SELECT password FROM user WHERE password = "'.$password.'"');
        $resultP = $comparerP->rowCount();
        if($resultP == 0){
            
            echo '<h3>Il\'y a un probleme avec Votre MDP</h>';
        } else {
            
            $user = $comparer->fetchAll(PDO::FETCH_ASSOC);
	...
	<div class="content_fullwidth">
      ...
      <div class="login_form two">		
			<form id="sky-form" class="sky-form" method="post">
                            <header><strong style="color:red">I</strong>nsta<strong style="color:red">M</strong>ail<strong style="color:red">M</strong>edia</header>
...
<h1>Chosse de user</h1>
    <?php } elseif (userIsAdmin() == TRUE) { ?> 
<h1>Chosse de Admin</h1>
    <?php } elseif (userIsSuperAdmin() == TRUE) { ?>
<h1>Chosse de SuperAdmin</h1>
    <?php } ?>
